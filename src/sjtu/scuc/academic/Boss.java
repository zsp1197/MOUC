...
//            tielines.setTielines((solveTielines(deltaSysLoad, "initialize")));
    ...
    public double[][][] solveTielines_cplex(double[][] deltaSysLoad, String mode) throws IloException {
        IloCplex cplex;
        IloNumVar[][][] x;
        IloNumVar[][] f;
        cplex = new IloCplex();
        final int no_of_sys = systems.size();
        final int no_of_ti = systems.get(0).getTiNum();
        x = new IloNumVar[no_of_sys][no_of_sys][no_of_ti];
        f = new IloNumVar[no_of_sys][no_of_ti];
        //        1
        for (int i = 0; i < no_of_sys; i++) {
            for (int j = 0; j < no_of_sys; j++) {
                for (int t = 0; t < no_of_ti; t++) {
//                        x[i][j][t] = model.addVar(-tielines.getTieMax()[i][j], tielines.getTieMax()[i][j], 0.0, GRB.CONTINUOUS, null);
//                        x[i][j]=cplex.numVarArray(no_of_ti, -tielines.getTieMax()[i][j], tielines.getTieMax()[i][j], IloNumVarType.Float);
                        x[i][j][t]=cplex.numVar( -tielines.getTieMax()[i][j], tielines.getTieMax()[i][j], IloNumVarType.Float);
                        if(i==j){
...
//        3
        for (int t = 0; t < no_of_ti; t++) {
            for (int i = 0; i < no_of_sys; i++) {
                ...
                cplex.addLe(expr,Math.abs(deltaSysLoad[i][t]), null);
                cplex.addGe(expr,-Math.abs(deltaSysLoad[i][t]), null);
...
//                f[i][t] = model.addVar(-MAX_VALUE, MAX_VALUE, 0, GRB.CONTINUOUS, null);
                f[i][t]=cplex.numVar( -MAX_VALUE, MAX_VALUE, IloNumVarType.Float);
                IloLinearNumExpr expr = cplex.linearNumExpr();
                for (int j = 0; j < no_of_sys; j++) {
                    expr.addTerm( x[i][j][t],mprices[i][t]);
                }
                cplex.addEq(expr,f[i][t]);
//                model.addConstr(expr, GRB.EQUAL, f[i][t], null);
        ...
        if(mode=="min"){
            ...
            cplex.addMaximize(expr, "cost");
        }
        else {
            throw new java.lang.Error("FUCK!");
        }
...
//        model.optimize();
        ...
        for (int t = 0; t < no_of_ti; t++) {
            for (int i = 0; i < no_of_sys; i++) {
                for (int j = 0; j < no_of_sys; j++) {
                    result[i][j][t] = cplex.getValue(x[i][j][t]);
//                    result[i][j][t] = x[i][j][t].get(GRB.DoubleAttr.X);
                }
            }
        }
        System.out.println("计算结果是"+Double.toString(cplex.getBestObjValue()));
        cplex.end();
        return result;
    }
...
//        3
        ...
        if (mode == "initialize") {
            ...
            System.out.println("计算结果是"+Double.toString(model.get(GRB.DoubleAttr.ObjVal)));
            model.dispose();
            env.dispose();
            return result;
        } else {
    ...
    public double[][][] solveTielines_4test(double[][] deltaSysLoad, String mode) throws GRBException {
        GRBEnv env = new GRBEnv();
        GRBModel model = new GRBModel(env);
        final int no_of_sys = systems.size();
        final int no_of_ti = systems.get(0).getTiNum();

...
//        find f=f1^2+f2^2
        ...
        if(calresult.getTargetflag()!=3){
            f=new DerivativeStructure(1, f1.multiply(f1), 1, f2.multiply(f2));
            throw new java.lang.Error("haven't done yet");
        }else {
    ...
    private boolean miss_Wrong(GeneratorWithQuadraticCostCurve gen, int[][] genStatus, double[][] genOutput, int i, int t) {
        if(genStatus[i][t]==0){
            return true;
        }
